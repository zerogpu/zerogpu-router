...
Defaults to `all-minilm-l6-v2` (22.7M parameters, 256-token window), the general-purpose choice for semantic similarity over short chunks. Use `-m bge-small-en-v1.5` instead (33.4M parameters, 512-token window) when the user asks for it, for English retrieval, for chunks longer than 256 tokens, or when retrieval quality is the bottleneck. Both cost \$0.50 per 1M input tokens and bill nothing on output, and both return 384-dimensional vectors, so they are interchangeable in an existing index.
