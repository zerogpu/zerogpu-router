---
name: zerogpu-router
description: Route cheap AI tasks (classification, summarization, entity/JSON extraction, PII redaction/extraction, follow-up question generation, small-model chat) to ZeroGPU small/nano models instead of spending host-model tokens. Invoke whenever the user asks for one of these tasks and the input is plain text.
metadata: { "openclaw": { "requires": { "mcp": ["zerogpu"] } } }
---
