...
- For non-interactive use (CI), when the user supplies a key, run `zerogpu login --api-key zgpu-api-…` instead. Avoid passing the key as a literal argument on shared machines, where it may land in shell history or process listings.
