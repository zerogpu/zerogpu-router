---
name: chat-glm
description: Chat with glm-5.2, a 753B MoE flagship with a 262K-token context window. Use when the input is too large for the other chat skills — an entire repository, a book-length document, a long agent transcript, though chat-glm-5-3-flash and chat-deepseek-v4-1-flash each hold four times as much — or for long-horizon reasoning the smaller models cannot hold together. This is the most expensive model on the platform, roughly 7x the cost of `chat`, so prefer chat for anything that fits in its 131K context.
argument-hint: "<text>"
