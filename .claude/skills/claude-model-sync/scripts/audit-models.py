#!/usr/bin/env python3
"""Audit the ZeroGPU Router Claude Code plugin against the live model catalog API.
...
section — that skill's own models. Cost ratios ("roughly 7x") are not checked; sweep
for them by hand whenever a price moves.

Usage:
  python3 .claude/skills/claude-model-sync/scripts/audit-models.py
  python3 .claude/skills/claude-model-sync/scripts/audit-models.py --model glm-5.2
  python3 .claude/skills/claude-model-sync/scripts/audit-models.py --save models.json
  python3 .claude/skills/claude-model-sync/scripts/audit-models.py --json models.json   # offline
  python3 .claude/skills/claude-model-sync/scripts/audit-models.py --strict             # CI/loop
"""
...
SKILLS_DIR = "agents/claude/skills"
README = "agents/claude/README.md"
...
COUNTS = [
    ...
    (README, re.compile(r"all (\d+) skills")),
]
...
SKILL_REF = re.compile(r"/zerogpu-router:([\w-]+)|`([\w-]+)`")
SKILL_SECTION = re.compile(r"^###\s+`/zerogpu-router:([\w-]+)`")
...
def plugin_files(root):
    """Every plugin file a model id can appear in, CHANGELOG excluded (it is history)."""
    out = []
    for base in PLUGIN_PATHS:
        for dirpath, dirnames, filenames in os.walk(os.path.join(root, base)):
            ...
            for fn in sorted(filenames):
                ...
                if fn.endswith((".md", ".json")) and rel != CHANGELOG:
                    out.append(rel)
    return out
...
def locations(root, mid, skills, rename):
    ...
    for rel in plugin_files(root):
        ...
        if skill and mid in skills.get(skill, {}):
            ...
            note = (
                "(replace every mention: Model/Wraps lines, tables, examples)"
                if rename
                else "(strip its section or model option, its reference-table row, and every mention)"
            )
        else:
        ...
        for n, line in enumerate(read_lines(root, rel)):
            if rel == README:
                if line.startswith("## "):
                    ...
                    m = SKILL_SECTION.match(line)
                    own = m.group(1) if m and m.group(1) in skills else None
            about = {mid for mid in known_ids if mentions(line, mid)}
            if own:
                about |= set(skills[own])
            for a, b in SKILL_REF.findall(line):
                about |= set(skills.get(a or b, {}))
        ...
        for rel, pattern in COUNTS:
            for n, line in enumerate(read_lines(root, rel)):
                for raw in pattern.findall(line):
                    if int(raw) != len(skills):
                        print(f"COUNT: {rel}:{n + 1}: says {raw} skills — agents/claude/skills has {len(skills)}")
                        findings += 1
